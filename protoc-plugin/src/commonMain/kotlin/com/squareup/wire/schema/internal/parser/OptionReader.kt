...
class OptionReader(internal val reader: SyntaxReader) {
    ...
    fun readOptions(): List<OptionElement> {
        if (!reader.peekChar('[')) return listOf()
    ...
    fun readOption(keyValueSeparator: Char): OptionElement {
        ...
        if (subName != null) {
            value = OptionElement(subName, kind, value)
            kind = Kind.OPTION
        }
        return OptionElement(name, kind, value, isParenthesized)
    }
     ...
     */
    private fun readMap(openBrace: Char, closeBrace: Char, keyValueSeparator: Char): Map<String, Any> {
        ...
        while (true) {
            ...
            if (value is OptionElement) {
                var nested: MutableMap<String, Any>? = result[name] as MutableMap<String, Any>
                if (nested == null) {
    ...
    private fun addToList(list: MutableList<Any>, value: Any) {
        if (value is List<*>) {
