...
class SchemaLoader {
        ...
        Closer.create().use { closer ->
            ...
            for (source in sources) {
                if (Files.isRegularFile(source)) {
                    val sourceFs = FileSystems.newFileSystem(source, javaClass.getClassLoader())
                    closer.register(sourceFs)
                    directories[source] = getOnlyElement(sourceFs.rootDirectories)
                } else {
    ...
    private fun loadDescriptorProto(): ProtoFile {
        ...
        resourceAsStream.source().buffer().use({ buffer ->
    ...
    companion object {
        private val DESCRIPTOR_PROTO = "google/protobuf/descriptor.proto"
